...
describe('Address', function() {
  ...
  var str = AddrUtils.bitcoin_address_to_jemcash('jLyuAhEgFtWEqBHNpLFCs2FqSoBYEPWJsf');
      ...
      if (!d[2].isPrivkey) {
        it('should describe address ' + d[0] + ' as valid', function() {
          ...
          if(type == 'pubkeyhash' || type == 'scripthash')
            test_addr = AddrUtils.bitcoin_address_to_jemcash(d[0]);
  ...
  describe('validation', function() {
    ...
    it('isValid returns true on a valid address', function() {
      var valid = Address.isValid(AddrUtils.bitcoin_address_to_jemcash('37BahqRsFrAd3qLiNNwLNV3AWMRD7itxTo'), 'livenet');
      valid.should.equal(true);
    });

    it('isValid returns false on network mismatch', function() {
      var valid = Address.isValid(AddrUtils.bitcoin_address_to_jemcash('37BahqRsFrAd3qLiNNwLNV3AWMRD7itxTo'), 'testnet');
      valid.should.equal(false);
    });

    it('validates correctly the P2PKH test vector', function() {
      for (var i = 0; i < PKHLivenet.length; i++) {
        var error = Address.getValidationError(AddrUtils.bitcoin_address_to_jemcash(PKHLivenet[i]));
    ...
    it('validates correctly the P2SH test vector', function() {
      for (var i = 0; i < P2SHLivenet.length; i++) {
        var error = Address.getValidationError(AddrUtils.bitcoin_address_to_jemcash(P2SHLivenet[i]));
        should.not.exist(error);
      }
    });
    ...
    it('validates correctly the P2SH testnet test vector', function() {
      for (var i = 0; i < P2SHTestnet.length; i++) {
        var error = Address.getValidationError(AddrUtils.bitcoin_address_to_jemcash(P2SHTestnet[i]), 'testnet');
        should.not.exist(error);
      }
    });
    ...
    it('rejects correctly the P2PKH livenet test vector with "testnet" parameter', function() {
      for (var i = 0; i < PKHLivenet.length; i++) {
        var error = Address.getValidationError(AddrUtils.bitcoin_address_to_jemcash(PKHLivenet[i]), 'testnet');
        should.exist(error);
      }
    });
    ...
    it('validates correctly the P2PKH livenet test vector with "livenet" parameter', function() {
      for (var i = 0; i < PKHLivenet.length; i++) {
        var error = Address.getValidationError(AddrUtils.bitcoin_address_to_jemcash(PKHLivenet[i]), 'livenet');
        should.not.exist(error);
      }
    });
    ...
    it('should not validate on a network mismatch', function() {
      ...
      for (i = 0; i < PKHLivenet.length; i++) {
        error = Address.getValidationError(AddrUtils.bitcoin_address_to_jemcash(PKHLivenet[i]), 'testnet', 'pubkeyhash');
        should.exist(error);
        error.message.should.equal('Address has mismatched network type.');
      }
      for (i = 0; i < PKHTestnet.length; i++) {
        error = Address.getValidationError(AddrUtils.bitcoin_address_to_jemcash(PKHTestnet[i]), 'livenet', 'pubkeyhash');
        should.exist(error);
        error.message.should.equal('Address has mismatched network type.');
      }
    });
    ...
    it('should not validate on a type mismatch', function() {
      for (var i = 0; i < PKHLivenet.length; i++) {
        var error = Address.getValidationError(AddrUtils.bitcoin_address_to_jemcash(PKHLivenet[i]), 'livenet', 'scripthash');
        should.exist(error);
    ...
    it('testnet addresses are validated correctly', function() {
      for (var i = 0; i < PKHTestnet.length; i++) {
        var error = Address.getValidationError(AddrUtils.bitcoin_address_to_jemcash(PKHTestnet[i]), 'testnet');
        should.not.exist(error);
      }
    });

    it('addresses with whitespace are validated correctly', function() {
      var ws = AddrUtils.bitcoin_address_to_jemcash('  \r \t    \n 1A6ut1tWnUq1SEQLMr4ttDh24wcbJ5o9TT \t \n            \r');
      var error = Address.getValidationError(ws);
      should.not.exist(error);
      Address.fromString(ws).toString().should.equal(AddrUtils.bitcoin_address_to_jemcash('1A6ut1tWnUq1SEQLMr4ttDh24wcbJ5o9TT'));
    });
  });
  ...
  describe('encodings', function() {
    ...
    it('should make this address from a compressed pubkey', function() {
      ...
      address.toString().should.equal(AddrUtils.bitcoin_address_to_jemcash('19gH5uhqY6DKrtkU66PsZPUZdzTd11Y7ke'));
    });
    ...
    it('should make this address from an uncompressed pubkey', function() {
      ...
      a.toString().should.equal(AddrUtils.bitcoin_address_to_jemcash('16JXnhxjJUhxfyx4y6H4sFcxrgt8kQ8ewX'));
      var b = new Address(pubkey, 'livenet', 'pubkeyhash');
      b.toString().should.equal(AddrUtils.bitcoin_address_to_jemcash('16JXnhxjJUhxfyx4y6H4sFcxrgt8kQ8ewX'));
    });
    ...
    describe('from a script', function() {
      ...
      it('should make this address from a p2pkh output script', function() {
        ...
        a.toString().should.equal(AddrUtils.bitcoin_address_to_jemcash('1KK9oz4bFH8c1t6LmighHaoSEGx3P3FEmc'));
        var b = new Address(s, 'livenet');
        b.toString().should.equal(AddrUtils.bitcoin_address_to_jemcash('1KK9oz4bFH8c1t6LmighHaoSEGx3P3FEmc'));
      });
      ...
      it('should make this address from a p2sh input script', function() {
        ...
        var a = Address.fromScript(s, 'livenet');
        a.toString().should.equal(AddrUtils.bitcoin_address_to_jemcash('3GueMn6ruWVfQTN4XKBGEbCbGLwRSUhfnS'));
        var b = new Address(s, 'livenet');
        b.toString().should.equal(AddrUtils.bitcoin_address_to_jemcash('3GueMn6ruWVfQTN4XKBGEbCbGLwRSUhfnS'));
      });

      it('returns the same address if the script is a pay to public key hash out', function() {
        var address = AddrUtils.bitcoin_address_to_jemcash('16JXnhxjJUhxfyx4y6H4sFcxrgt8kQ8ewX');
      ...
      it('returns the same address if the script is a pay to script hash out', function() {
        var address = AddrUtils.bitcoin_address_to_jemcash('3BYmEwgV2vANrmfRymr1mFnHXgLjD6gAWm');
    ...
    it('should derive from this known address string testnet', function() {
      var a = new Address(AddrUtils.bitcoin_address_to_jemcash(PKHTestnet[0]), 'testnet');
      var b = new Address(a.toString());
      b.toString().should.equal(AddrUtils.bitcoin_address_to_jemcash(PKHTestnet[0]));
      b.network.should.equal(Networks.testnet);
    });

    it('should derive from this known address string livenet scripthash', function() {
      var a = new Address(AddrUtils.bitcoin_address_to_jemcash(P2SHLivenet[0]), 'livenet', 'scripthash');
      var b = new Address(a.toString());
      b.toString().should.equal(AddrUtils.bitcoin_address_to_jemcash(P2SHLivenet[0]));
    });

    it('should derive from this known address string testnet scripthash', function() {
      var address = new Address(AddrUtils.bitcoin_address_to_jemcash(P2SHTestnet[0]), 'testnet', 'scripthash');
      address = new Address(address.toString());
      address.toString().should.equal(AddrUtils.bitcoin_address_to_jemcash(P2SHTestnet[0]));
    });
  ...
  describe('#toBuffer', function() {
    ...
    it('3c3fa3d4adcaf8f52d5b1843975e122548269937 corresponds to hash jLyuAhEgFtWEqBHNpLFCs2FqSoBYEPWJsf', function() {
      var address = new Address(str);
  ...
  describe('#toString', function() {
    ...
    it('scripthash address', function() {
      var address = new Address(AddrUtils.bitcoin_address_to_jemcash(P2SHLivenet[0]));
      address.toString().should.equal(AddrUtils.bitcoin_address_to_jemcash(P2SHLivenet[0]));
    });

    it('testnet scripthash address', function() {
      var address = new Address(AddrUtils.bitcoin_address_to_jemcash(P2SHTestnet[0]));
      address.toString().should.equal(AddrUtils.bitcoin_address_to_jemcash(P2SHTestnet[0]));
    });

    it('testnet pubkeyhash address', function() {
      var address = new Address(AddrUtils.bitcoin_address_to_jemcash(PKHTestnet[0]));
      address.toString().should.equal(AddrUtils.bitcoin_address_to_jemcash(PKHTestnet[0]));
    });
  ...
  describe('#inspect', function() {
    it('should output formatted output correctly', function() {
      var address = new Address(str);
      var output = '<Address: jLyuAhEgFtWEqBHNpLFCs2FqSoBYEPWJsf, type: pubkeyhash, network: livenet>';
  ...
  describe('questions about the address', function() {
    it('should detect a P2SH address', function() {
      new Address(AddrUtils.bitcoin_address_to_jemcash(P2SHLivenet[0])).isPayToScriptHash().should.equal(true);
      new Address(AddrUtils.bitcoin_address_to_jemcash(P2SHLivenet[0])).isPayToPublicKeyHash().should.equal(false);
      new Address(AddrUtils.bitcoin_address_to_jemcash(P2SHTestnet[0])).isPayToScriptHash().should.equal(true);
      new Address(AddrUtils.bitcoin_address_to_jemcash(P2SHTestnet[0])).isPayToPublicKeyHash().should.equal(false);
    });
    it('should detect a Pay To PubkeyHash address', function() {
      new Address(AddrUtils.bitcoin_address_to_jemcash(PKHLivenet[0])).isPayToPublicKeyHash().should.equal(true);
      new Address(AddrUtils.bitcoin_address_to_jemcash(PKHLivenet[0])).isPayToScriptHash().should.equal(false);
      new Address(AddrUtils.bitcoin_address_to_jemcash(PKHTestnet[0])).isPayToPublicKeyHash().should.equal(true);
      new Address(AddrUtils.bitcoin_address_to_jemcash(PKHTestnet[0])).isPayToScriptHash().should.equal(false);
    });
  });
  ...
  it('can roundtrip from/to a object', function() {
    var address = new Address(AddrUtils.bitcoin_address_to_jemcash(P2SHLivenet[0]));
    expect(new Address(address.toObject()).toString()).to.equal(AddrUtils.bitcoin_address_to_jemcash(P2SHLivenet[0]));
  });
  ...
  describe('creating a P2SH address from public keys', function() {
    ...
    it('can create an address from a set of public keys', function() {
      ...
      address.toString().should.equal(AddrUtils.bitcoin_address_to_jemcash('3FtqPRirhPvrf7mVUSkygyZ5UuoAYrTW3y'));
    ...
    it('works on testnet also', function() {
      ...
      address.toString().should.equal(AddrUtils.bitcoin_address_to_jemcash('2N7T3TAetJrSCruQ39aNrJvYLhG1LJosujf'));
    });
    ...
    it('can create an address from a set of public keys with a nested witness program', function() {
      ...
      address.toString().should.equal(AddrUtils.bitcoin_address_to_jemcash('3PpK1bBqUmPK3Q6QPSUK7BQSZ1DMWL6aes'));
    });
