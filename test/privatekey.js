...
describe('PrivateKey', function() {
  ...
  var wifTestnet = AddrUtils.bitcoin_address_to_jemcash('cSdkPxkAjA4HDr5VHgsebAPDEh9Gyub4HK8UJr2DFGGqKKy4K5sG');
  var wifTestnetUncompressed = AddrUtils.bitcoin_address_to_jemcash('92jJzK4tbURm1C7udQXxeCBvXHoHJstDXRxAMouPG1k1XUaXdsu');
  var wifLivenet = AddrUtils.bitcoin_address_to_jemcash('L2Gkw3kKJ6N24QcDuH4XDqt9cTqsKTVNDGz1CRZhk9cq4auDUbJy');
  var wifLivenetUncompressed = AddrUtils.bitcoin_address_to_jemcash('5JxgQaFM1FMd38cd14e3mbdxsdSa9iM2BV6DHBYsvGzxkTNQ7Un');
  ...
  it('should create a private key from WIF string', function() {
    var a = new PrivateKey(AddrUtils.bitcoin_address_to_jemcash('L3T1s1TYP9oyhHpXgkyLoJFGniEgkv2Jhi138d7R2yJ9F4QdDU2m'));
    should.exist(a);
    should.exist(a.bn);
  });

  it('should create a private key from WIF buffer', function() {
    var a = new PrivateKey(Base58Check.decode(AddrUtils.bitcoin_address_to_jemcash('L3T1s1TYP9oyhHpXgkyLoJFGniEgkv2Jhi138d7R2yJ9F4QdDU2m')));
    should.exist(a);
    should.exist(a.bn);
  });
    ...
    validbase58.map(function(d){
      if (d[2].isPrivkey) {
        it('should instantiate WIF private key ' + d[0] + ' with correct properties', function() {
          ...
          var key = new PrivateKey(AddrUtils.bitcoin_address_to_jemcash(d[0]));
  ...
  describe('instantiation', function() {
    ...
    it('should not be able to instantiate private key because of network mismatch', function() {
      expect(function() {
        return new PrivateKey(AddrUtils.bitcoin_address_to_jemcash('L3T1s1TYP9oyhHpXgkyLoJFGniEgkv2Jhi138d7R2yJ9F4QdDU2m'), 'testnet');
      }).to.throw('Private key network mismatch');
    });

    it('should not be able to instantiate private key WIF is too long', function() {
      expect(function() {
        var buf = Base58Check.decode(AddrUtils.bitcoin_address_to_jemcash('L3T1s1TYP9oyhHpXgkyLoJFGniEgkv2Jhi138d7R2yJ9F4QdDU2m'));
  ...
  describe('#toAddress', function() {
    it('should output this known livenet address correctly', function() {
      var privkey = PrivateKey.fromWIF(AddrUtils.bitcoin_address_to_jemcash('L3T1s1TYP9oyhHpXgkyLoJFGniEgkv2Jhi138d7R2yJ9F4QdDU2m'));
      var address = privkey.toAddress();
      address.toString().should.equal(AddrUtils.bitcoin_address_to_jemcash('1A6ut1tWnUq1SEQLMr4ttDh24wcbJ5o9TT'));
    });

    it('should output this known testnet address correctly', function() {
      var privkey = PrivateKey.fromWIF(AddrUtils.bitcoin_address_to_jemcash('cR4qogdN9UxLZJXCNFNwDRRZNeLRWuds9TTSuLNweFVjiaE4gPaq'));
      var address = privkey.toAddress();
      address.toString().should.equal(AddrUtils.bitcoin_address_to_jemcash('mtX8nPZZdJ8d3QNLRJ1oJTiEi26Sj6LQXS'));
    });

    it('creates network specific address', function() {
      var pk = PrivateKey.fromWIF(AddrUtils.bitcoin_address_to_jemcash('cR4qogdN9UxLZJXCNFNwDRRZNeLRWuds9TTSuLNweFVjiaE4gPaq'));
  ...
  describe('#getValidationError', function(){
    ...
    it('should validate as true', function() {
      var a = PrivateKey.isValid(AddrUtils.bitcoin_address_to_jemcash('L3T1s1TYP9oyhHpXgkyLoJFGniEgkv2Jhi138d7R2yJ9F4QdDU2m'));
      a.should.equal(true);
    });
  ...
  describe('#toPublicKey', function() {
    ...
    it('should convert this known PrivateKey to known PublicKey and preserve compressed=true', function() {
      var privwif = AddrUtils.bitcoin_address_to_jemcash('L3T1s1TYP9oyhHpXgkyLoJFGniEgkv2Jhi138d7R2yJ9F4QdDU2m');
    ...
    it('should convert this known PrivateKey to known PublicKey and preserve compressed=false', function() {
      var privwif = AddrUtils.bitcoin_address_to_jemcash('92jJzK4tbURm1C7udQXxeCBvXHoHJstDXRxAMouPG1k1XUaXdsu');
  ...
  it('creates an address as expected from WIF, livenet', function() {
    var privkey = new PrivateKey(AddrUtils.bitcoin_address_to_jemcash('5J2NYGstJg7aJQEqNwYp4enG5BSfFdKXVTtBLvHicnRGD5kjxi6'));
    privkey.publicKey.toAddress().toString().should.equal(AddrUtils.bitcoin_address_to_jemcash('135bwugFCmhmNU3SeCsJeTqvo5ViymgwZ9'));
  });

  it('creates an address as expected from WIF, testnet', function() {
    var privkey = new PrivateKey(AddrUtils.bitcoin_address_to_jemcash('92VYMmwFLXRwXn5688edGxYYgMFsc3fUXYhGp17WocQhU6zG1kd'));
    privkey.publicKey.toAddress().toString().should.equal(AddrUtils.bitcoin_address_to_jemcash('moiAvLUw16qgrwhFGo1eDnXHC2wPMYiv7Y'));
  });
