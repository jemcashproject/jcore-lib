...
describe('Script', function() {
  ...
  describe('#buildPublicKeyHashOut', function() {
    it('should create script from livenet address', function() {
      var address = Address.fromString(AddrUtils.bitcoin_address_to_jemcash('1NaTVwXDDUJaXDQajoa9MqHhz4uTxtgK14'));
      ...
      s.toAddress().toString().should.equal(AddrUtils.bitcoin_address_to_jemcash('1NaTVwXDDUJaXDQajoa9MqHhz4uTxtgK14'));
    });
    it('should create script from testnet address', function() {
      var address = Address.fromString(AddrUtils.bitcoin_address_to_jemcash('mxRN6AQJaDi5R6KmvMaEmZGe3n5ScV9u33'));
      var s = Script.buildPublicKeyHashOut(address);
      should.exist(s);
      s.toString().should.equal('OP_DUP OP_HASH160 20 0xb96b816f378babb1fe585b7be7a2cd16eb99b3e4 OP_EQUALVERIFY OP_CHECKSIG');
      s.isPublicKeyHashOut().should.equal(true);
      s.toAddress().toString().should.equal(AddrUtils.bitcoin_address_to_jemcash('mxRN6AQJaDi5R6KmvMaEmZGe3n5ScV9u33'));
    });
  ...
  describe('#buildScriptHashOut', function() {
    ...
    it('inherits network property from other script', function() {
      var s1 = new Script.fromAddress(new Address(AddrUtils.bitcoin_address_to_jemcash('1FSMWkjVPAxzUNjbxT52p3mVKC971rfW3S')));
    ...
    it('inherits network property form an address', function() {
      var address = new Address(AddrUtils.bitcoin_address_to_jemcash('34Nn91aTGaULqWsZiunrBPHzFBDrZ3B8XS'));
  ...
  describe('getData returns associated data', function() {
    ...
    it('for a P2SH address', function() {
      var address = Address.fromString(AddrUtils.bitcoin_address_to_jemcash('3GhtMmAbWrUf6Y8vDxn9ETB14R6V7Br3mt'));
  ...
  describe('toAddress', function() {
    ...
    it('for a P2PKH address', function() {
      var stringAddress = AddrUtils.bitcoin_address_to_jemcash('1NaTVwXDDUJaXDQajoa9MqHhz4uTxtgK14');
      var address = new Address(stringAddress);
      var script = new Script(address);
      script.toAddress().toString().should.equal(stringAddress);
    });
    it('for a P2SH address', function() {
      var stringAddress = AddrUtils.bitcoin_address_to_jemcash('3GhtMmAbWrUf6Y8vDxn9ETB14R6V7Br3mt');
      var address = new Address(stringAddress);
      var script = new Script(address);
      script.toAddress().toString().should.equal(stringAddress);
    });
    ...
    it('works for p2pkh output', function() {
      ...
      script.toAddress().toString().should.equal(AddrUtils.bitcoin_address_to_jemcash('1KK9oz4bFH8c1t6LmighHaoSEGx3P3FEmc'));
    });
    it('works for p2pkh input', function() {
      ...
      s2.toAddress().toString().should.equal(AddrUtils.bitcoin_address_to_jemcash('17VArX6GRE6i6MVscBUZoXwi6NhnHa68B7'));
    });
    ...
    it('works for p2sh output', function() {
      ...
      script.toAddress().toString().should.equal(AddrUtils.bitcoin_address_to_jemcash('3FiMZ7stbfH2WG5JQ7CiuzrFo7CEnGUcAP'));
    });
    it('works for p2sh input', function() {
      ...
      script.toAddress().toString().should.equal(AddrUtils.bitcoin_address_to_jemcash('3GYicPxCvsKvbJmZNBBeWkC3cLuGFhtrQi'));
    });
